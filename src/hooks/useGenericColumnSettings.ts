...
import { GenericColumnConfig } from '@/components/common/ColumnSettings';
...
export const useGenericColumnSettings = (pageName: string, defaultColumns: GenericColumnConfig[]) => {
  const [columns, setColumns] = useState<GenericColumnConfig[]>(defaultColumns);
  ...
  const saveSettings = useCallback(async (newColumns: GenericColumnConfig[]) => {
