...
export interface GenericColumnConfig {
  key: string;
  label: string;
  visible: boolean;
  width: number;
...
interface ColumnSettingsProps {
  columns: GenericColumnConfig[];
  onColumnChange: (columns: GenericColumnConfig[]) => void;
  defaultColumns?: GenericColumnConfig[];
  excludeFromWidthControl?: string[]; // Keys to exclude from width slider (e.g., 'photo', 'actions')
}
...
const ColumnSettings = ({ 
  ...
  const handleHideAll = () => {
    ...
    const updated = columns.map(col => ({ 
      ...col, 
      visible: col.key === 'actions' ? true : false 
    }));
    onColumnChange(updated);
  };

  const widthControlColumns = columns.filter(
    col => col.visible && !excludeFromWidthControl.includes(col.key)
  );

  const visibilityColumns = columns.filter(
    col => !excludeFromWidthControl.includes(col.key)
  );
