...
export type ReminderColumnKey = 'color' | 'title' | 'remarks' | 'createdBy' | 'status' | 'dateCreated' | 'lastUpdated' | 'actions';

export interface ReminderColumnConfig {
  key: ReminderColumnKey;
  label: string;
  visible: boolean;
  width: number;
  minWidth: number;
...
interface ColumnSettingsProps {
  ...
  onColumnChange: (columns: ReminderColumnConfig[]) => void;
  defaultColumns?: ReminderColumnConfig[];
}

const ReminderColumnSettings = ({ columns, onColumnChange, defaultColumns }: ColumnSettingsProps) => {
  const [open, setOpen] = useState(false);

  const handleVisibilityChange = (key: ReminderColumnKey, visible: boolean) => {
  ...
  const handleWidthChange = (key: ReminderColumnKey, width: number) => {
    const updated = columns.map(col => 
...
export default ReminderColumnSettings;