...
import { X, Loader2, Plus, ZoomIn, ZoomOut, RotateCcw } from 'lucide-react';
...
interface MedCertPhotoUploadCellProps {
  recordId: string;
  photos: string[];
  maxPhotos?: number;
  onPhotoUpdate: () => void;
...
export const MedCertPhotoUploadCell = ({ 
  recordId, 
  photos = [], 
  maxPhotos = 3,
  onPhotoUpdate 
}: MedCertPhotoUploadCellProps) => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `${recordId}-med-cert-${Date.now()}.${fileExt}`;
      const filePath = `${fileName}`;

      const { error: uploadError } = await supabase.storage
        .from('med-cert-photos')
      ...
      const { error: updateError } = await supabase
        ...
        .update({ med_cert_photos: updatedPhotos })
        .eq('id', recordId);

      if (updateError) throw updateError;

      toast.success('Med Cert photo uploaded successfully');
      onPhotoUpdate();
    } catch (error: any) {
    ...
    <div className="flex items-center gap-1">
      ...
      {currentPhotos.map((photo, index) => (
        <div key={index} className="relative group">
          ...
          >
            <img src={photo} alt={`Med Cert ${index + 1}`} className="w-full h-full object-cover" />
          </button>
          {canUpload && (
            <button
        ...
        <DialogContent className="sm:max-w-[700px] max-h-[90vh]">
          <DialogHeader>
            <div className="flex items-center justify-between">
              <DialogTitle>Med Cert Photo</DialogTitle>
              <div className="flex items-center gap-1">
